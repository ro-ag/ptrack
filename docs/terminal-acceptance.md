...
| AG-1 | Agent CLIs | Open each discovered installed agent (Agy, Claude Code, Codex, Cursor Agent, Gemini, Kimi Code, OpenCode) and exercise one harmless interactive prompt without granting a capability. | UI remains interactive; discovery installs or authenticates nothing; terminal close cleans up the process. | Where available |
...
| IN-1 | Mouse | Open a disposable full-screen terminal application; move, click, drag, and scroll inside the pane. | Mouse coordinates/actions update and no board action fires through the terminal. | macOS, Windows, Linux |
...
| RD-1 | Unicode and emoji | Print combining, wide, emoji, flag, and box-drawing fixtures with **Settings → Terminal → Unicode mode** set to modern graphemes and to compatibility (the change applies to open panes). | Combining, wide, emoji, and flag fixtures remain legible; enabled mode aligns the sample separators. | macOS, Windows, Linux |
...
| ST-1 | Sustained output | Stream 100 MiB from a disposable local producer; scroll away from the bottom and monitor the app process. | Output completes, UI remains responsive, queued transport stays bounded, scrollback stays within the selected profile bound, and the viewport does not jump. | macOS, Windows, Linux |
...
```sh
cargo test -p ptrack-terminal --all-targets
cargo test -p ptrack-app --lib
cargo clippy -p ptrack-terminal -p ptrack-app --all-targets -- -D warnings
