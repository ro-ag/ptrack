...
impl Sha256 {
    ...
    pub(crate) fn update(&mut self, mut bytes: &[u8]) {
        self.byte_len = self
            .byte_len
            .checked_add(u64::try_from(bytes.len()).expect("usize fits in u64"))
            .expect("migration bundle length is bounded");
        if self.block_len != 0 {
    ...
    pub(crate) fn finish(mut self) -> [u8; 32] {
        let bit_len = self
            .byte_len
            .checked_mul(8)
            .expect("bundle length is bounded");
...
fn compress(state: &mut [u32; 8], block: &[u8; 64]) {
    ...
    for (word, chunk) in schedule[..16].iter_mut().zip(block.chunks_exact(4)) {
        *word = u32::from_be_bytes(chunk.try_into().expect("chunk is four bytes"));
    }
    for index in 16..64 {
