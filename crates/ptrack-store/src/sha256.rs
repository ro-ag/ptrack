...
pub fn digest(bytes: &[u8]) -> [u8; 32] {
    ...
    for block in &mut chunks {
        compress(&mut state, block.try_into().expect("SHA-256 block length"));
    }
    ...
    let bit_length = u64::try_from(bytes.len())
        .expect("Rust slices cannot exceed u64 on supported targets")
        .checked_mul(8)
        .expect("bounded migration values cannot overflow their SHA-256 length");
    final_blocks[final_length - 8..final_length].copy_from_slice(&bit_length.to_be_bytes());
...
fn compress(state: &mut [u32; 8], block: &[u8; 64]) {
    ...
    for (word, bytes) in schedule[..16].iter_mut().zip(block.chunks_exact(4)) {
        *word = u32::from_be_bytes(bytes.try_into().expect("SHA-256 word length"));
    }
    for index in 16..64 {
